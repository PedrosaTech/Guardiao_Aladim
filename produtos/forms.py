...
class ProdutoForm(forms.ModelForm):
    """Formulário para dados globais do produto. codigo_interno editável via campo extra."""
    ...
    def clean_codigo_interno(self):
        ...
        qs = Produto.objects.filter(codigo_interno=codigo, is_active=True)
        if self.instance.pk:
...
class ProdutoParametrosEmpresaForm(forms.ModelForm):
    """Parâmetros comerciais e fiscais do produto para uma empresa."""
    ...
    class Meta:
        ...
        widgets = {
            'cfop_venda_dentro_uf': forms.TextInput(attrs={'class': 'form-control'}),
            'cfop_venda_fora_uf': forms.TextInput(attrs={'class': 'form-control'}),
            'csosn_cst': forms.TextInput(attrs={'class': 'form-control'}),
            'icms_st_cst': forms.TextInput(attrs={'class': 'form-control'}),
            'pis_cst': forms.TextInput(attrs={'class': 'form-control'}),
            'cofins_cst': forms.TextInput(attrs={'class': 'form-control'}),
            'ipi_venda_cst': forms.TextInput(attrs={'class': 'form-control'}),
            'ipi_compra_cst': forms.TextInput(attrs={'class': 'form-control'}),
            'cclass_trib': forms.TextInput(attrs={'class': 'form-control'}),
    ...
    def __init__(self, *args, **kwargs):
        ...
        for name, field in self.fields.items():
            if 'class' in getattr(field.widget, 'attrs', {}):
                continue
...
class CodigoBarrasAlternativoForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        empresa_ids = kwargs.pop('empresa_ids', None)
